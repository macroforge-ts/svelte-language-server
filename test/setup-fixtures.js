...
const fixturesDir = path.join(
    __dirname,
    'plugins/typescript/features/diagnostics/fixtures'
);

// exports-map-svelte fixture
const exportsMapSvelte = path.join(
    fixturesDir,
    'exports-map-svelte/node_modules/package'
);
fs.mkdirSync(exportsMapSvelte, { recursive: true });

fs.writeFileSync(
    path.join(exportsMapSvelte, 'package.json'),
    JSON.stringify(
        {
            name: 'package',
            version: '1.0.0',
...
fs.writeFileSync(
    path.join(exportsMapSvelte, 'foo.svelte'),
    '<script lang="ts">export let foo: string;</script>\n'
);

fs.writeFileSync(
    path.join(exportsMapSvelte, 'x.svelte'),
    '<script lang="ts">export let x: number;</script>\n'
);

fs.writeFileSync(
    path.join(exportsMapSvelte, 'x-types.d.ts'),
    'declare const X: any;\nexport default X;\n'
);

fs.writeFileSync(
    path.join(exportsMapSvelte, 'y.svelte'),
