...
describe('HTML Plugin', () => {
    ...
    it('provides hover info', async () => {
        ...
        assert.deepStrictEqual(
            plugin.doHover(document, Position.create(0, 2)),
            <Hover> {
                contents: {
                    kind: 'markdown',
    ...
    it('does not provide completions inside of moustache tag', async () => {
        ...
        const tagCompletion = plugin.doTagComplete(
            document,
            Position.create(0, 20)
        );
    ...
    it('does not provide rename for element being uppercase', async () => {
        ...
        assert.deepStrictEqual(
            plugin.rename(document, Position.create(0, 2), 'p'),
            null
        );
    });
    ...
    it('does not provide rename for valid element but incorrect position #1', () => {
        ...
        assert.deepStrictEqual(
            plugin.prepareRename(document, Position.create(0, 16)),
            null
        );
        assert.deepStrictEqual(
            plugin.prepareRename(document, Position.create(0, 5)),
            null
        );
        assert.deepStrictEqual(
            plugin.prepareRename(document, Position.create(0, 26)),
            null
        );

        assert.deepStrictEqual(
            plugin.rename(document, Position.create(0, 16), newName),
            null
        );
        assert.deepStrictEqual(
            plugin.rename(document, Position.create(0, 5), newName),
            null
        );
        assert.deepStrictEqual(
            plugin.rename(document, Position.create(0, 26), newName),
            null
        );
    });
    ...
    it('does not provide rename for valid element but incorrect position #2', () => {
        ...
        assert.deepStrictEqual(
            plugin.prepareRename(document, Position.create(0, 33)),
            null
        );
        assert.deepStrictEqual(
            plugin.prepareRename(document, Position.create(0, 36)),
            null
        );

        assert.deepStrictEqual(
            plugin.rename(document, Position.create(0, 33), newName),
            null
        );
        assert.deepStrictEqual(
            plugin.rename(document, Position.create(0, 36), newName),
            null
        );
    });
    ...
    it('provides rename for element', () => {
        ...
        const pepareRenameInfo = Range.create(
            Position.create(0, 1),
            Position.create(0, 4)
        );
        assert.deepStrictEqual(
        ...
        assert.deepStrictEqual(
            plugin.rename(document, Position.create(0, 2), newName),
            renameInfo
        );
        assert.deepStrictEqual(
    ...
    it('provides linked editing ranges', async () => {
        ...
        const ranges = plugin.getLinkedEditingRanges(
            document,
            Position.create(0, 3)
        );
