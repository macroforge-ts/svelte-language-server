...
export class DiagnosticsProviderImpl implements DiagnosticsProvider {
    ...
    async getDiagnostics(
        ...
        diagnostics = diagnostics
            ...
            .filter(
                (diagnostics) => !expectedTransitionThirdArgument(diagnostics, tsDoc, lang)
            );
...
function rangeMapper(
    ...
    return (diagnostic) => {
        ...
        if (range.start.line < 0) {
            range = movePropsErrorRangeBackIfNecessary(
                diagnostic,
                snapshot,
...
function expectedTransitionThirdArgument(
    ...
    const callExpression = ts.isIdentifier(node) && ts.isCallExpression(node.parent)
        ? node.parent
        : findNodeAtSpan(
            node,
